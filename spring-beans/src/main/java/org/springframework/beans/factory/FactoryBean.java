...
/**
 * 由在{@link BeanFactory}中使用的对象实现的接口，这些对象本身是
 * 单个对象的工厂。如果一个bean实现了此接口，它将用作暴露对象的工厂，
 * 而不是直接作为将被暴露的bean实例本身。
 *
 * <p><b>注意：实现此接口的bean不能用作普通bean。</b>
 * FactoryBean以bean风格定义，但为bean引用暴露的对象
 * ({@link #getObject()})始终是它创建的对象。
 *
 * <p>FactoryBeans可以支持单例和原型，可以按需延迟创建对象，
 * 也可以在启动时立即创建。{@link SmartFactoryBean}接口允许暴露
 * 更细粒度的行为元数据。
 *
 * <p>此接口在框架内部被大量使用，例如用于AOP的
 * {@link org.springframework.aop.framework.ProxyFactoryBean}或
 * {@link org.springframework.jndi.JndiObjectFactoryBean}。它也可以用于
 * 自定义组件；然而，这只在基础设施代码中常见。
 *
 * <p><b>{@code FactoryBean}是一个程序化契约。实现不应该依赖于注解驱动的注入
 * 或其他反射工具。</b>{@link #getObjectType()}和{@link #getObject()}的调用
 * 可能在引导过程的早期到达，甚至在任何后处理器设置之前。如果你需要访问其他bean，
 * 实现{@link BeanFactoryAware}并以编程方式获取它们。
 *
 * <p><b>容器只负责管理FactoryBean实例的生命周期，而不是FactoryBean创建的对象的生命周期。</b>
 * 因此，暴露的bean对象上的销毁方法（如{@link java.io.Closeable#close()}）
 * 将<i>不会</i>被自动调用。相反，FactoryBean应该实现{@link DisposableBean}，
 * 并将任何此类关闭调用委托给底层对象。
 *
...
public interface FactoryBean<T> {
	 ...
	 * <p>与{@link BeanFactory}一样，这允许支持单例和原型设计模式。
	 * <p>如果此FactoryBean在调用时尚未完全初始化（例如因为它涉及循环引用），
	 * 则抛出相应的{@link FactoryBeanNotInitializedException}。
	 * <p>允许FactoryBeans返回{@code null}对象。bean工厂将把它视为
	 * 要使用的正常值，在这种情况下不会抛出{@code FactoryBeanNotInitializedException}。
	 ...
	 * 它应该提前估计类型。
	 * 对于原型，建议在此处返回有意义的类型。
	 * <p>此方法可以在FactoryBean完全初始化之<i>前</i>调用。
	 ...
	 * 普通的{@link FactoryBean}实现如果不实现此扩展接口，则
	 * 简单地假定如果
