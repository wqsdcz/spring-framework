...
public class ConvertingComparator<S, T> implements Comparator<S> {
	...
	/**
	 * 创建一个新的 {@link ConvertingComparator}，基于 {@linkplain java.util.Map.Entry#getKey() 键}
	 * 来比较 {@linkplain java.util.Map.Entry 映射条目}。
	...
	/**
	 * 创建一个新的 {@link ConvertingComparator}，基于 {@linkplain java.util.Map.Entry#getValue() 值}
	 * 来比较 {@linkplain java.util.Map.Entry 映射条目}。
	...
	/**
	 * 将 {@link ConversionService} 和 {@code targetType} 适配为 {@link Converter}。
	 */
	private static class ConversionServiceConverter<S, T> implements Converter<S, T> {
		...
		public ConversionServiceConverter(ConversionService conversionService, Class<? extends T> targetType) {
			Assert.notNull(conversionService, "ConversionService 不可为 null");
			Assert.notNull(targetType, "'targetType' 不可为 null");
